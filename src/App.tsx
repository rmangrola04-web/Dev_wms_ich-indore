...
import { PackingMaterialView } from './views/PackingMaterialView';

export default function App() {
  // Theme state (Earthy Warm Mode by default: Ancient Pottery, Indian Painting, Beige Flash, etc.)
  const [theme, setTheme] = useState<'wms-dark' | 'light' | 'earthy-warm'>(() => {
    try {
      const saved = localStorage.getItem('wms_theme');
      if (['wms-dark', 'light', 'earthy-warm'].includes(saved as string)) return saved as any;
    } catch(e) {}
    return 'earthy-warm';
  });
  ...
  useEffect(() => {
    ...
    if (theme === 'wms-dark') {
      ...
      htmlEl.setAttribute('data-theme', 'wms-dark');
    } else if (theme === 'earthy-warm') {
      htmlEl.classList.add('light');
      bodyEl.classList.add('light');
      htmlEl.setAttribute('data-theme', 'earthy-warm');
    } else {
      htmlEl.classList.add('light');
      bodyEl.classList.add('light');
  ...
  return (
    <div className="h-[100dvh] w-full max-w-full overflow-hidden flex flex-col selection:bg-blue-600 selection:text-white bg-[var(--bg-main)]">
      <div className="flex flex-1 min-h-0 w-full max-w-full overflow-hidden">
            ...
            <div className="space-y-1">
              {isSecurity ? (
                ...
                <>
                  ...
                  <button
                    onClick={() => handleSwitchView('packingMaterialView')}
                    className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all ${
                      activeView === 'packingMaterialView'
                        ? 'bg-blue-600 text-white shadow-sm shadow-blue-500/25'
                        : 'text-slate-600 hover:bg-slate-100/80 hover:text-slate-900'
                    }`}
                  >
                    <Package className={`w-4 h-4 ${activeView === 'packingMaterialView' ? 'text-white' : 'text-amber-600'}`} />
                    <span>Packing Material & Thermocol</span>
                  </button>
        ...
        {/* Main Workspace Area */}
        <div className="flex-1 flex flex-col min-w-0 w-full max-w-full overflow-x-hidden bg-[var(--bg-main)] text-slate-900 ">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center gap-1">
                <select
                  ...
                  className="bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-300 text-xs rounded-xl px-3 py-2 font-bold focus:outline-none focus:ring-1 focus:ring-amber-600 cursor-pointer transition shadow-2xs"
                  title="Select WMS Theme Mode"
                >
                  <option value="earthy-warm">🌍 Earthy Warm Mode</option>
                  <option value="wms-dark">🌙 WMS Dark Mode</option>
          ...
          {/* Main Content Views */}
          <main className="flex-1 overflow-y-auto [-webkit-overflow-scrolling:touch] px-1.5 py-2 sm:px-4 md:px-6 pb-[calc(env(safe-area-inset-bottom)+90px)] md:pb-6 space-y-3 sm:space-y-5 w-full max-w-full">
            {isSecurity ? (
              <GateSecurityView
        ...
        <footer className="h-9 bg-white  border-t border-slate-200  px-4 md:px-6 flex justify-between items-center text-[10px] text-slate-500  shrink-0">
          <div className="flex items-center gap-3">
      ...
      <div className="mobile-bottom-nav mobile-bottom-dock hidden max-md:flex md:hidden fixed bottom-0 left-0 right-0 w-full max-w-full px-1.5 pt-2 pb-[calc(env(safe-area-inset-bottom)+8px)] justify-around items-center z-[9999] overflow-x-auto no-scrollbar bg-slate-900/95 border-t border-sky-500/40 shadow-2xl backdrop-blur-lg">
        {isSecurity ? (
          <button
            onClick={() => handleSwitchView('gateSecView')}
